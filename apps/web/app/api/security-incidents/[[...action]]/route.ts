...
/**
 * T2b — Proxy passthrough /api/security-incidents*  → NestJS API.
 * Uses [[...action]] so the base path GET /api/security-incidents also resolves.
 */
...
async function passthrough(
  ...
  const targetUrl = `${API_URL}/api/security-incidents${suffix}${url.search}`;

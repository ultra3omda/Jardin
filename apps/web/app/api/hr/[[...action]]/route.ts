...
/**
 * T2c — RH/Paie : passthrough proxy /api/hr* (contrats, congés, paie) → NestJS API.
 * Uses [[...action]] so the base path GET /api/hr also resolves.
 */
...
async function passthrough(
  ...
  const targetUrl = `${API_URL}/api/hr${suffix}${url.search}`;

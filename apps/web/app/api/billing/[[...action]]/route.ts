...
/** V7-E — Passthrough proxy `/api/billing*` → NestJS `/api/billing*`. */
...
async function passthrough(
  ...
  const target = `${API_URL}/api/billing${suffix}${url.search}`;
  ...
  // Binary PDF passthrough (invoice download) — never text-decode it.

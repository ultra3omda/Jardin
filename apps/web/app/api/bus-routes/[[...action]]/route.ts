...
/**
 * T2b — Proxy passthrough /api/bus-routes*  → NestJS API.
 * Uses [[...action]] so the base path GET /api/bus-routes also resolves.
 */
...
async function passthrough(
  ...
  const targetUrl = `${API_URL}/api/bus-routes${suffix}${url.search}`;

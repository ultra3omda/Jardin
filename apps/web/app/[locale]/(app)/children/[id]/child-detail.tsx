...
export function ChildDetail({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/dashboard" className="text-sm font-medium text-primary hover:underline">← Mes enfants</Link>

      <header className="flex items-center gap-4">
        <span className="flex h-14 w-14 items-center justify-center rounded-full bg-pink-100 text-lg font-semibold text-pink-700">
          {initials}
        </span>
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-navy-900">{child.firstName} {child.lastName}</h1>
          <p className="text-sm text-muted-foreground">{child.className ?? 'Classe non assignée'}</p>
        </div>
      </header>

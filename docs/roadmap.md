...
| **1.8** | **Super_admin UI provisioning école** : `/admin/tenants` (list + create + detail + resend-invite), endpoint `POST /admin/tenants` atomique (tenant + admin + invite + Resend), réutilise V1.5 InviteTokens + V1.6 InviteEmail template. M1. | ~2j | V1.7-A2 + V1.6 | ✅ Livré 2026-05-25 |
| **2** | **Module Élèves** — Web full CRUD (liste + création sectionnée 4 sections + détail + edit + delete + bulk-import CSV) + Mobile read-only (liste FlatList + détail sectionné + tile dashboard) + RBAC 5 rôles × 7 actions + photo R2 + isolation R10 étendue. 15 champs Complet (D24). | ~6j | V1.8 | ✅ Livré 2026-05-25 |
| **Landing publique** | klasso.tn/ bilingue FR/AR — 7 sections (Hero · Bénéfices · Modules · Confiance · Pricing · Form démo · Footer) + endpoint `POST /api/public/demo-request` (Resend + Cloudflare Turnstile invisible + Throttler 5/h/IP) + migration routing complète sous `app/[locale]/` (next-intl v4 sub-paths). Première surface commerciale publique pour démos écoles. | ~1.7j | V2 + klasso.tn DNS | ✅ Livré 2026-05-26 |
| **Landing V0.5** | **Tunisian Editorial UX upgrade** — palette OKLCH (terracotta + cream + deep teal), typo Fraunces + Public Sans + Markazi Text + IBM Plex Sans Arabic via `next/font`, 5 nouvelles sections (Stats · SchoolSegments · DashboardMockup · FAQ · CTAFinal), 6 atoms réutilisables (DropCap · ScrollReveal · CountUp · DeckleEdge · ZelligePattern · Section), hero photo SVG placeholder + duotone CSS, hero reveal motion-driven, scroll-reveal CSS `animation-timeline: view()` + IO fallback, ADR 0008 (D26 lock = chiffres occidentaux partout). | ~3.5j | Landing publique | ✅ Livré 2026-05-26 (PR #31) |
| **Landing V0.7** | **Photos réelles + tests atoms** — install React Testing Library + jest-dom + jsdom dans `apps/web`, vitest env switch node → jsdom + setup file, réintroduction des unit tests `DropCap` et `CountUp` (couvre rendering + className forwarding + reduced-motion / IO stub). `next.config.mjs` `images.remotePatterns` étendu à `images.unsplash.com` + Hero swap SVG placeholder → photo Aaron Burden CC0 (Vercel Image Optimizer pour resize/format). | ~0.5j | Landing V0.5 | 🚧 PR V0.7 en cours 2026-05-26 |
| **3-A** | **Lien parent ↔ élève (N-N)** — Modèle Prisma `ParentStudent` (RelationType: FATHER/MOTHER/LEGAL_GUARDIAN/OTHER + isPrimaryContact) sur User(role=PARENT) (D27/D28), `ParentRelationsModule` API (CRUD + RBAC + tenant scope) + 8 unit tests, proxy Next `/api/parent-relations` + client TanStack + `<ParentsSection>` sur `/students/[id]`. ADR 0009 locks decisions. **Communication 1:1 reportée V3-B** (Socket.IO d'entrée — D29). | ~1.5j | V2 | ✅ Livré 2026-05-26 (PR #34) |
| **3-B** | **Communication 1:1 Socket.IO** — Prisma `Conversation` + `ConversationParticipant` + `Message`, NestJS `MessagingGateway` (`/messaging` namespace, JWT handshake auth, rooms `user:<id>`), REST mirror endpoints + HTTP fallback, web chat UI (`/messages` list + `/messages/[id]` thread, TanStack Query + `socket.io-client`), direct browser→Railway WS (bypass Vercel proxy). Mobile chat reporté V12. ADR 0010. | ~3j | V3-A | 🚧 PR V3-B en cours 2026-05-26 |
| **4-A** | **Classes + Affectations enseignants + EDT (recurring weekly slots, D31)** — Prisma `Class` + `ClassTeacher` + `TimeSlot` models, backend CRUD complet (10 endpoints, SCHOOL_ADMIN gated), proxy Next `/api/classes` + client TanStack, page `/classes` (list + create) + `/classes/[id]` (detail + EDT weekly grid 7 jours × 8h-18h avec slots cliquables-supprimables). ADR 0011 (Choix EDT model + UI custom grid). | ~2j | V2 | 🚧 PR V4-A en cours 2026-05-26 |
| **4-B** | **EDT polish + Teacher dashboard** — Drag-and-drop slots, vue "Mes classes" persona TEACHER, bulk EDT CSV import, Student → classId FK migration progressive. | ~1j | V4-A | 📋 Planifiée |
...
| **6-A** | **Pédagogie + Bulletins PDF (D32 lock — @react-pdf/renderer)** — Prisma `Subject` + `GradePeriod` + `Evaluation` + `Grade` + `Bulletin` models, 4 NestJS modules (subjects/grade-periods/evaluations/bulletins) avec TEACHER scope par classe + isClosed lock, service `BulletinPdfService` (renderToBuffer → A4 Helvetica), pages web `/classes/[id]/grades` (saisie auto-save) + `/students/[id]/bulletin` (génération + download PDF), ADR 0012. 24 unit tests Vitest. | ~3j | V4-A | ✅ Livré 2026-05-27 |
| **6-B** | **Pédagogie polish** — Pondération matière + coefficient évaluation, commentaire enseignant sur bulletin, accès lecture parent (notes + téléchargement bulletin), persona TEACHER "Mes classes", upload R2 + URL persistée, rapports trimestriels agrégés. | ~2j | V6-A | 📋 Planifiée |
...
| **V7-C** | **Hotfix prod** — Self-healing demo-login backend (auto-seeds demo tenants + users on first call to fix DEMO_USER_NOT_SEEDED 404 on klasso.tn prod) + mobile Expo Web deploy config (vercel.json rewrites + docs/MOBILE_WEB_DEPLOY.md instructions for separate `klasso-mobile.vercel.app` project). | ~0.3j | V7-B | ✅ Livré 2026-05-27 |
| **8** | Finance : Facturation parents, paiements en ligne (Stripe + Konnect), relances | 3j | V2 + V3 + Resend (V1.5) | 📋 Planifiée |
| **9** | Stock + Cantine + Transport + Santé (carnet médical) + Sécurité (incidents) | 3j | V2 + V4 | 📋 Planifiée |
| **10** | Notifications multi-canal : push (Expo) + email (Resend) + SMS (Twilio) + WhatsApp Business (option) + queues BullMQ/Upstash | 2j | V3 + V5 + Upstash | 📋 Planifiée |
| **11** | Admin SaaS : super-admin platform, billing tenants (subscription Stripe), analytics plateforme (PostHog) | 2j | V8 | 📋 Planifiée |
| **12** | Hardening : Sentry complet + Better Stack + Postgres RLS + perf audit + audit RGPD + backup strategy + **custom domain par école** (`portail.ecole-xyz.fr` via API Vercel automation + admin cross-tenant + brand audit log + optionnel M2 EAS Build natif per-tenant, voir D20). Subdomain wildcard `*.klasso.tn` désormais en V1.7. | ~3j | tout V1-11 + V1.7 | 📋 Planifiée |
| **13** | Mobile build & soumission stores : EAS Build + screenshots + TestFlight + Google Play Internal Testing | 3j | tout V1-12 | 📋 Planifiée |
...
| **Custom domain OVH** | **Q3=C** — Repoussé en V11 hardening. On garde `ecole-saas-weld.vercel.app` + `api.klasso.tn` jusque-là. | V11 |
...
| D22 | **Cloud-first preview locked (V1.7-A2)** | **Décision utilisateur 2026-05-24** : tout composant doit être visible/testable en production via une URL cloud accessible depuis n'importe quel navigateur, sans setup local. <br/>**État existant à reconnaître (non touché)** : <br/>• **API** déjà en prod depuis V1 (2026-05-21) sur Railway projet `ecole-saasapi-production` — build via **Nixpacks auto-détecté** (PAS de Dockerfile commit). URL publique `https://api.klasso.tn`, `/health` + `/api/docs` répondent 200. Auto-deploy sur push to `main`. <br/>• **Web** déjà en prod depuis V0 sur Vercel projet `ecole-saas`. URL `https://ecole-saas.vercel.app`. <br/>**Net nouveau V1.7-A2** : <br/>• **Mobile (Expo Web)** : nouveau projet Vercel `klasso-mobile` (id `prj_rS0q3PqBBh90fJYTdMvxsv6rmOwM`), URL `https://klasso-mobile.vercel.app`. Build CI-side via `.github/workflows/deploy-mobile.yml` (install monorepo → build shared → `expo export --platform web` → `vercel deploy apps/mobile/dist`). Auto-deploy sur push to `main` (paths `apps/mobile/**` ou `packages/shared/**`). Trigger manuel : `gh workflow run deploy-mobile.yml`. <br/>• **Pnpm monorepo + Expo Metro** : ajout `.npmrc` `node-linker=hoisted` (requis par Metro bundler en monorepo, voir https://docs.expo.dev/guides/monorepos/#pnpm-workspaces). Non-régression vérifiée pour `apps/web` + `apps/api`. <br/>**Hors scope** : aucune modification API/Railway — la stack actuelle (Nixpacks + Railway dashboard config) reste source de vérité. Si Dockerfile devient nécessaire un jour (perf, custom system deps), faire ADR dédiée. <br/>**Local dev mobile** : 2 paths supportés : (a) Expo Go sur smartphone via QR scan (Wi-Fi LAN), (b) Cloud preview URL Vercel (zero install). Path (a) reste documenté dans `apps/mobile/README.md` pour les modules futurs qui ont besoin de modules natifs. | Toute fonctionnalité livrée doit avoir une URL cloud accessible — pas de demo qui demande au user de tirer le repo et installer les deps. Aligné avec la philosophie « production-grade dès V0 » du CLAUDE.md. **Règle invariante** : avant d'ajouter un Dockerfile/config CI/script deploy, vérifier l'état cloud existant via `curl /health` + roadmap (composants déployés depuis V1). |
...
| D25 | **Landing publique bilingue FR/AR + V0 commercial activation** | **Décision utilisateur 2026-05-25** : (1) Style éditorial **institutionnel bilingue FR/AR** (vs SaaS startup moderne FR-only) — alignement marché tunisien + scaling MENA. (2) Routing **`/fr` et `/ar` sub-paths** (vs cookie) — SEO bilingue indexé Google. (3) **Pricing par élève/mois** : Starter 5 TND (≤50) · Standard 4 TND (≤200) · Pro 3 TND (illimité). Standard tier featured. (4) **Cloudflare Turnstile invisible** anti-spam (vs honeypot). (5) **Tagline "L'école à l'ère numérique"** — sobre, ancrage marché TN. <br/>**Ordre vagues décidé** : Landing → Mobile stores (TestFlight + Play Internal) → V3 Parents. <br/>**Email demo destinataire** : variable `process.env.DEMO_REQUEST_TO_EMAIL`. Initial `ultra3omda@gmail.com`, futur `demo@klasso.tn` après Google Workspace (zéro redéploiement nécessaire). <br/>**Wildcard `*.klasso.tn` non-activable** : Vercel ne valide pas wildcard CNAME via DNS externe + switch Vercel DNS casse mail OVH. Reporté V11 (avec Google Workspace migration simultanée). Path-based `/t/<slug>/` reste l'URL tenant V0. <br/>**Hors-scope V0 landing** : témoignages clients (zéro client en prod, ne pas inventer), vidéo démo (V2.1+), FAQ remplie (V1), dark mode (V2.1+), analytics PostHog (V11), Lighthouse audit + a11y screen reader (à exécuter en preview Vercel avant promotion), hero image (gradient suffisant V0, image optimization V0.5). <br/>**Effort réel** : ~1.7j sur 28 tasks bite-sized exécutés via subagent-driven-development pattern (3 deviations API mineures absorbées : next-intl v4 `requestLocale: Promise`, ResendService `template` field, 5 V1.6 tenant shims à patcher). Voir ADR 0007. | Surface commerciale `klasso.tn/` dispo + infra i18n posée pour V3+ bilingue. Form démo → pipeline manuel super-admin (Resend → email) suffit pour 10-20 prospects/sem. Coût ~1.7j vs ROI : capture prospects + SEO indexé FR/AR. |
| D24 | **V2 Module Élèves — scope étendu Complet + bulk + photo + mobile read-only** | **Décision utilisateur 2026-05-25** : (1) **Profondeur champs = "Complet" (~15)** vs "Minimal" (6) — identité (+ nationalité, langue), scolarité, famille, contact (adresse, ville, CP, pays), santé light (medicalNotes + warning RGPD bandeau ambre), photo. (2) **Plateformes = Web full CRUD + Mobile read-only** (mobile write reportée V3). (3) **RBAC = Standard métier** : SCHOOL_ADMIN write, TEACHER+STAFF read all, PARENT read own children (scoped server-side par `parentEmail = currentUser.email`), SUPER_ADMIN via `/admin/tenants`. (4) **+ Bulk CSV import** : `POST /students/bulk-import` avec `dryRun` mode, atomicité forte (1 erreur ⇒ 0 insert), 1000 lignes / 5 MB cap. <br/>**Effort réel** : ~6j (vs 3j initial roadmap) — scope Complet + bulk + photo + mobile. <br/>**Hors-scope explicite** : entité `Class` relationnelle (→ V4), relation `Parent` N-N (→ V3), médical strict PHI (→ V8), CSV templates UI téléchargeables (→ V11), recherche full-text (→ V11), mobile write CRUD (→ V3), export PDF (→ V11). <br/>**Tests** : 9 unit + ~16 e2e (CRUD + RBAC matrix complète + bulk-import + photo + isolation R10 étendue Student). <br/>**Photo** : réutilise pattern V1.6 `R2Service.signedPutUrl` zéro nouvel infra. Voir ADR 0006. | Premier module métier livré clé en main, démos commerciales TN/MENA possibles immédiatement. La dette technique (parentEmail string, classroom string, medicalNotes free text) est documentée et migrée par les vagues correspondantes (V3/V4/V8). |
...
| 2026-05-27 | Claude Code + ultra3omda | **V7-A Design Refactor livré** — nouvelle vague V7 insérée entre V6 et V8 Finance (ancien V7) qui glisse à V8. Toutes les vagues subséquentes glissées de +1 (V8→V9, V9→V10, V10→V11, V11→V12, V12→V13). V7-A web inclut : design tokens navy/ambre/paper/ink + Tailwind extend, demo-login API NestJS (`POST /api/auth/demo-login` rate-limited 60/h/IP + 6 unit tests), seed étendu (2 tenants démo + 9 users + ~50 students), nav matrix `getNavForUser()` (5 rôles × 3 tenant types + 7 unit tests), dashboard config `getDashboardConfig()` (8 variantes persona × type), 4 composants dashboard (KpiCard + QuickAction + NotesPanel + AnnouncementsPanel), 4 composants app-shell (Sidebar navy + Topbar + UserPill + NavSection), DemoAccountsBlock (8 personas 1-clic), auth layout 2-col + login V7, AppShellClient refactor, landing TopNav sticky + 4 anchors + section IDs. ADR 0013. V7-B mobile à venir dans plan séparé. |

...
| **1.6** | **White-label runtime per-tenant** (fondations, sans domaine custom) : `Tenant.brand JSON` + endpoints API admin + bucket R2 logos + injection CSS vars **post-auth** (`(app)/layout.tsx` après `getMe()`) + routes **path-based pré-auth** `/t/[tenantSlug]/{login,register,forgot-password,reset-password,verify-email}` + page settings + emails brandés Resend + type partagé `TenantBrand` (mobile theme consommé V2) + ADR 0003. Décision **D20** révisée PM 2026-05-22 (no-custom-domain). **Zéro action DNS user, 0 €/mois additionnel.** | ~1.5j | V1.5 | 📋 Planifiée |
...
| **11** | Hardening : Sentry complet + Better Stack + Postgres RLS + perf audit + audit RGPD + backup strategy + **white-label subdomain + custom domain** (wildcard DNS `*.ecole-saas.com` après achat domaine + middleware Next.js host-resolver + `POST /api/admin/tenant/custom-domain` via API Vercel + admin cross-tenant + brand audit log + optionnel M2 EAS Build natif per-tenant, voir D20) | ~4j | tout V1-10 + V1.6 | 📋 Planifiée |
| **12** | Mobile build & soumission stores : EAS Build + screenshots + TestFlight + Google Play Internal Testing | 3j | tout V1-11 | 📋 Planifiée |
| | **Total restant** | **~34 jours** (V11 + 1j pour subdomain repoussé en PM) | | |
